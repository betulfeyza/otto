...
from otto_phase1.predict import TARGETS, predictions_to_submission_df
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate LightGBM models on archive/test_parquet with test_labels"
    )
    parser.add_argument("--data-root", type=Path, default=Path("archive"))
    parser.add_argument(
        "--model-dir", type=Path, default=Path("outputs/lgb_models_full_v1")
    )
    parser.add_argument(
        "--submission-out",
        type=Path,
        default=Path("outputs/submission_lgb_full_v1.csv"),
    )
    parser.add_argument(
        "--max-train-shards",
        type=int,
...
def _load_models(model_dir: Path) -> Dict[str, lgb.Booster]:
    ...
    for target in TARGETS:
        model_path = model_dir / f"lgb_model_{target}.txt"
...
def main() -> None:
    ...
    print("[1/5] Loading LightGBM models...")
    ...
    print("[4/5] Predicting test shards with LightGBM...")
